...
import qmini_tasks.tasks  # noqa: F401
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    with launch_simulation(env_cfg, args_cli):
        ...
        # Play needs only the actor: skip the critic/optimizer (privileged, training-only) so a checkpoint
        # still loads after the critic obs group changes -- otherwise its critic shape mismatches the
        # current env cfg and the load fails, even though the actor is unaffected.
        runner.load(
            resume_path,
